  ...
  Widget _registerButton(BuildContext context){
    return Center(
      child: ElevatedButton(
        ...
        onPressed: (){
          setState(() {
            if(imageData.isEmpty || !emailController.text.contains("@") || !emailController.text.contains(".com") ||
                dniController.text.length < 8 || celularController.text.length < 9 || passwordController.text.length < 8  ||
              ...
              if(passwordController.text.length < 8){
                _alert.createAlert(context, 'Verificar contraseña', 'La contraseña es muy pequeña');
              }
              if(passwordController.text != passwordConfirmController.text){
                _alert.createAlert(context, 'Verificar contraseñas', 'Las contraseñas no coinciden');
              }
              if(imageData.isEmpty){
                _alert.createAlert(context, 'Falta imagen', 'Falta subir imagen de la licencia');
              }
            } else {
              jsonData = _usuarioProvider.registerS3(nombresController.text, aPaternoController.text, aMaternoController.text,
                  dniController.text, celularController.text, fechaNacController.text, emailController.text, passwordController.text, imageFile, context);
