...
import 'package:carsharing_app/utils/email.dart';
...
class UsuarioProvider {
    ...
    if(response.statusCode == 200){
      ...
      if(jsonData != null){
        ...
        if(sharedPreferences.getBool("enable") == false) {
          //TODO: MANDAR CORREO
          _email.sendEmail(jsonData['nombres'],  jsonData['correo'], jsonData['apellidoPaterno'], jsonData['apellidoMaterno']);
  ...
  /*register(String nombres, String aPaterno, String aMaterno, String dni,
      String celular, String fechaNac, String correo, String password, String image, BuildContext context, File file) async {
    ...
    var response = await http.post("$_url/s3", headers: {"Content-Type": "application/json" }, body: bodyRequest);
  ...
  update(int id, String nombres, String aPaterno, String aMaterno, String dni,
    ...
    var bodyRequest = json.encode(data);
    var jsonData;
    var response = await http.put("$_url$id", headers: {"Content-Type": "application/json" }, body: bodyRequest);
    if(response.statusCode == 200) {
      jsonData = json.decode(response.body);
      return jsonData;
    }
  }
  //METODO PARA REGISTRAR UN USUARIO EN S3
  registerS3(String nombres, String aPaterno, String aMaterno, String dni,
      String celular, String fechaNac, String correo, String password, File image, BuildContext context) async {

    var request = http.MultipartRequest("POST", Uri.parse("${_url}s3"));

    request.fields["nombres"] = nombres;
    request.fields["apellidoPaterno"] = aPaterno;
    request.fields["apellidoMaterno"] = aMaterno;
    request.fields["dni"] = dni;
    request.fields["correo"] = correo;
    request.fields["celular"] = celular;
    request.fields["password"] = password;
    request.fields["esAdm"] = "false";
    request.fields["enable"] = "false";
    request.fields["fechaNac"] = fechaNac;
